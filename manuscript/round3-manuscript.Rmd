...
I present two examples of this "forecast trap" and examine how it arises.
This disconcerting situation is possible whenever two conditions are met: (1) the optimal management policy is not unique to the generative process, and (2) the generative process is not included in candidate set of models.
...
This term thus includes not only process-based models, but could also statistical forecasting methods (e.g. ARIMA, @arima), non-parametric approaches such as empirical dynamical modeling [@Ye2015], or machine learning. 
...
Figure \ref{fig1} shows both the forecasts and realized management outcomes of two alternative three-species models, "A" and "B" (see Appendix A) in predicting the population dynamics of striped bass (an economically important fishery) and double-crested cormorants (a target species for conservation) which both feed on a population of river herring (whose abundance we assume is not measured). 
...
Model B produces substantially less accurate forecasts, but Model B's forecasts also lead to better outcomes, seen both in the population abundances and overall net utility.
...
In the scenario depicted in Figure \ref{fig1}, I have used a the richer five-species model introduced @Brias2021 to drive the underlying dynamics, which includes three competing species of herring that are preyed upon by both the bass and the cormorants. 
...
# Iterative updating: _Sequential Decision Processes_
...
In both cases, model 2 provides far more accurate forecasts, as seen in the error bars and confirmed by the distribution of proper scores [@Gneiting2007].
...
We can assess such outcomes in less abstract terms than forecasting skill, such as economic value them manager sought to optimize (in dollars) or the ecological value (unharvested biomass) [Fig 3].
...
## Integrated assessments and adaptive management
...
To avoid failure to explore sufficiently, [see exploration-exploitation tradeoff, e.g. @Walters1981],
...
The difference between the performance with and without learning is known as the "Value of Information".
...
( out[out$method=="learning","total"][[1]] -  out[out$method=="planning","total"][[1]]) / out[out$method=="planning","total"][[1]]
...
The forecast trap illustrated in both examples can best be understood as a problem of _non-uniqueness_.
...
In the second example, the "true" model is simple enough to be covered by a suitable candidate set of models (for instance, a Gaussian Process, @Boettiger2015).
Given sufficient data, @Gneiting2007's theorem regarding strictly proper scores guarantees we will eventually select the underlying model and achieve the optimal forecast and optimal decision.
...
The forecast trap demonstrates that over a given ensemble of candidate models, the value of information (VOI) can be negative. 
...
The policy derived without learning reflects greater uncertainty: it is more _robust_.


## A way forward

In practice, managers rarely rely only on forecasting skill to assess models, nor determine policies directly from forecasts alone, as in the SDP  [@Schindler2015].
Three principles already established in management practice but too often overlooked in research [@Schindler2015] can also help avert the forecast trap: (1) methods for articulating uncertainty 

 (a) better articulation of uncertainty; (b) more attention to which elements of uncertainty really matter to the decision; (c) methods to detect when the truth falls outside the model set; and (d) methods for generating strategies that are robust to the sort of uncertainty described here.

@Schindler2015 contrasts 
Many disciplines use scenario planning to assist policy in accounting for irreducible uncertainties [@Peterson2003], in which a range of methods, including model-based forecasts, can be used to determine plausible scenarios.

It is worth noting that quantitative and algorithmic approaches to decision-making need not be predicated on forecasting skill. 
Recently developed reinforcement learning which have been applied to Atari video-games, car racing, and nuclear fusion are based on "model-free" algorithms which make no attempt to learn and predict future states of the systems they control. 
Instead, the algorithm's neural networks are trained only to learn a  robust "policy", mapping observations to actions.
While a search space of policies: say, $n$ possible states by $m$ possible actions is potentially very large, it is always far smaller than the corresponding search space of forecasts: which for a time horizon of $T$ is $(m \cdot n^2)^T$.


a type of machine learning, rely on so-called "model free" approaches which do not involve 
 

Several ways to avoid the forecast trap are 


The forecast trap can be avoided by seeking policies which are robust to uncertainty [e.g. @Schindler2015].
...
A quantitative decision process does not need to be derived from a forecast

The forecast trap is a consequence of model non-uniqueness [@Konikow1992] of models with respect either to forecast skill or policy outcomes.

Models of any natural system can never be validated, because natural systems are never closed and models are not unique [@Oreskes1994].
...
Forecast skill (a unitless statistical measure) is clearly a different objective than policy outcomes (be they measured in dollars or fish in the ocean).
Yet the result is surprising all the same. 
The forecast is not just some other arbitrary modeling objective; it is a central input into the decision making process, both in the real world [@Clark2001; @Dietze2018] and in our idealized decision-making algorithm, SDP [@Marescot2013].

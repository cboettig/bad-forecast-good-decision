...
In practice, managers rarely rely only on forecasting skill to assess models, nor determine policies directly from forecasts alone, as in the SDP.
Such existing principles, frequently reflected in management practice can help avert the forecast trap, but are too often overlooked in research predicated on predict-and-prescribe approaches [@Schindler2015].
These include 
(a) emphasizing a better articulation of uncertainty a priori; 
(b) active exploration of new policies
...
Forecasts based on non-mechanistic models such as empirical dynamical modeling [EDM, Ye2015; @Brias2021] may help in articulating a broader ensemble of scenarios.





A manager may also escape the forecast trap by exploring actions which are not recommended from any of the available forecasts.
...
@Schindler2015 also underscores the value of flexible policies; rather than "managing solely within the range of past variation; active probing is usually needed."
@Schindler2015 contrasts this to a typical interpretation of the 'precautionary principle' often cited as a reason to avoid exploratory actions.
However, just because active exploration can escape the forecast trap does mean it is always a good idea.

Third, managers may emphasize policy robustness over expected net utility. 
...
This acts more like a sensitivity analysis of utility with respect to underlying assumptions, rather than an optimization routine.
...
If the stock size has gone up, increase harvest slightly, otherwise, decrease slightly.
...
a process known as curriculum learning [@], mimics the scenario analyses and search for . Such approaches have been used to train agents to play 2600 Atari console games at superhuman ability [@], outperform race-car drivers [@] and control nuclear fusion reactions [@].
Such approaches are also not yet well understood, and can fail dramatically [@].
...
The author acknowledges support from NSF CAREER Award #1942280 and helpful discussions with Melissa Chapman. The author is also deeply grateful to the Michael Runge and three other anonymous reviewers whose detailed feedback and insights have greatly shaped the paper and my own understanding of these issues.

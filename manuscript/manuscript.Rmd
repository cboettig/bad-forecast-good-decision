...
may consider a range of alternative plausible models[^1] to better reflect 
...
under what decisions or actions. This has made model-based forecasts a cornerstone for scientifically
based decision making.  But, do better models always lead to better decisions?

Whenever models are estimated from the data they seek to describe, there is a concern that
the model may over-fit the data: making the data seem even more likely than it would be
under the true process [@Burnham1998].  Over-fitting is a central concern of any model comparison
because over-fit models may underestimate uncertainty and make poor predictions about future
outcomes [@Ginzburg2004]. By comparing outcomes predicted by a model to future observations, 
a decision maker can not only _plan_ for the uncertainty, but also _learn_ which models are most trustworthy.  
This iterative learning process of learning will not only decrease uncertainty by narrowing 
beliefs over plausible model or parameter values, but can also eliminate models which overfit the data.
Because any iterative learning compares models to future outcomes, overfitting those outcomes
is impossible: a concept formalized rigorously by @Gneiting2007's proof of "proper" scoring rules.
By definition, under a proper scoring rule, no probabilistic prediction $Q(x)$ can score better, on
...
cannot be over-fit.  The ability to decrease uncertainty while avoiding over-fitting makes
such iterative learning particularly appealing approach to decision making in an uncertain
world [@Dietze2018], and the approach is already well established in the principle of 
adaptive management [@Walters1978] and existing practices such as Management Strategy Evaluation (MSE)
[@Punt2016] used in fisheries. Given this ability to decrease uncertainty and avoid over-fitting,
such iterative updating approaches seem irreproachable from a theoretical basis, even if
they may be difficult to execute in practice. 

However, as long as the true process model is unknown (effectively always), there is no
such theoretical guarantee that iterative learning over possible models or parameters will
lead to better outcomes.  This is not -- cannot -- be because some models have over-fit the 
data, as least as long as proper scoring rules are used for evaluation.  As I illustrate here,
given a set of candidate models, iterative learning either through assessment of forecast
skill or through formal adaptive management can lead to worse outcomes than forgoing any learning step.
Put another way, the value of information [VOI; @Howard1966; @Katz1987], as measured by the expected utility given that 
information minus the utility without it, can be negative: a rational decision maker ought to pay
not to know information even when that information is perfectly accurate.  
This situation is a direct consequence of the fact that a model need only capture certain
features of the underlying process to lead to an effective decision.


[^1]: While some authors distinguish between model uncertainty and parameter uncertainty, I will refer collectively to
any uncertainty that arises from our imperfect knowledge of the system as "model uncertainty," because the distinction
is largely a consequence of notation rather something more intrinsic.  For example, two structurally different models, $f(x) = a x$
and $f(x) = a x^2$ can be considered merely different parameterizations of $f(x) = a x^b$, or vice versa.  
...
Increasing the space of possible models to cover a whole plausible range of parameters $r$ and $K$ does nothing to resolve this problem.  Learning is somewhat slower over the larger range of possibilities, but nevertheless converges towards parameter values with lower growth rates and lower carrying capacity, for which the target biomass is far lower [Fig 4]. The manager becomes trapped in vicious cycle as biomass declines further and further, subsequent observations seem more and more consistent with models which most favor over-fishing in the first place.  As biomass declines, annual harvests and profits also plunge, and yet our manager becomes only more confident in their decisions.
...
am_multi %>%
  ...
  geom_line() + 
  geom_point() +
  geom_point(aes(time, harvest), col="darkgreen", shape=2) + 
  scale_colour_gradient(limits = c(0,1), low = pal[4], high = pal[1]) + 

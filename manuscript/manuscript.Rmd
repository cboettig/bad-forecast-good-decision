...
- abbreviated running title: "The trap of self-fulfilling prophecies"
...
In this paper, I demonstrate that the model that makes the best prediction is not necessarily the model that makes the best policy.
I show that iterative learning or iterative forecasting can be counter-productive to management outcomes:
...
As this example will show, by better understanding what aspects of a model are most and least important to shaping a particular decision, we can at least anticipate the circumstances that make this problem more likely.

Iterative learning approaches such as adaptive management or iterative forecasting [@Dietze2018] are particularly compelling because they address a different reason why seemingly-accurate models can lead to poor outcomes: the problem of overfitting.
When models are estimated from the data they seek to describe, the model may overfit the data, making the data seem even more likely than it would be under the true process [@Burnham1998].
Over-fit models may underestimate uncertainty and make poor predictions about future outcomes [@Ginzburg2004].
Because any iterative learning compares models to future outcomes, over-fitting those outcomes is impossible.
...
@Gneiting2007 further shows which techniques for evaluating forecast predictions do and do not satisfy this condition, i.e. which cannot be over-fit.
...
Previous work has long acknowledged the panoply of ways in which iterative learning or other model-based decision making can go astray due to conflicting incentives, implementation errors, or lack of resources for monitoring and updating [e.g. @Ludwig1993].
...
If such additional structurally different models are not enough, then perhaps yet more complex models would serve?
For this toy example, it would be possible to span all the models considered here, including our "true" model 3, using 
non-parametric approaches such as Gaussian Process models [@Boettiger2015].
Even this does not guarantee a solution, as iterative learning erodes belief in model 1, and prefers harvests that retain stock sizes near historic values where predictability is highest.
Besides, reality will always be more complex; we will never have the true model.
...
The premise that a model need not perfectly capture reality to be useful is at the very heart of modeling. Model 1 successfully captures the one key feature driving decisions in optimal harvest control problems: the stock size at which the maximum growth rate occurs [Fig 4A].
Capturing only the essential aspects as simply as possible is the goal of any model building exercise. 
Thus it should be no surprise that a model can drive good decisions while making poor predictions.
...
While iterations that revise these models will no doubt improve decision outcomes in many cases, it is worth bearing in mind from the example here that such a connection is not guaranteed.
<!-- So what? -->
...
We have seen that the problem cannot be resolved by more data, and is not the result of over-fitting.

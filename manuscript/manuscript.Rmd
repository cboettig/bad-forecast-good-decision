...
In both approaches, the manager quickly concludes that model 1 is entirely implausible, while finding that model 2 is remarkably accurate at predicting future values -- the manager is unknowing trapped into bad outcomes.
...
This simple example can be resolved by including our true model in the candidate set, or any class of models flexible enough to 'span' the true model, such as a Gaussian Process [e.g. see @Boettiger2015].  
Given sufficient data, either approach shown here will prefer the true model over any other (as guaranteed by @Gneiting2007's theorem), and thus discover the optimal policy.
While it is not difficult to image a candidate set of models flexible enough to include the very simple "true model" from Eq (4), it is much more unlikely that our model candidates truly span the generative processes of the real world.

...
under what decisions or actions.  Further, by comparing the outcomes predicted by a model
to future observations, a decision maker can not only _plan_ for the uncertainty, but 
also _learn_ which models are most trustworthy.  This iterative learning process is
particularly appealing because the models which best fit currently available data are not 
always the models which lead to the best decisions.  Such outcomes are often characterized
as the result of "over-fitting" [@Burnham2002]. While many methods (e.g. information criteria @Burnham2002)
are developed seeking to avoid this




While models
may also reflect uncertainties intrinsic to the process (stochasticity) or the limits 
of measurement (observation uncertainty), 


Why then do some models which most closely match available data lead to worse outcomes than models which appear less accurate?
This situation is almost universally considered to be the result of "over-fitting" [@], which much effort is dedicated to avoiding 
in selecting models. Because the defining flaw of an over-fit model is the failure to correctly predict additional data, 


<!-- even in a world absent of power politics, conflicting incentives, imperfect implementation, this is not an issue that can be addressed by statistics and data alone.  The 

-->


making approaches to uncertainty a central part
of effective decision-making [@Polasky2011; @Clark2001]. Reducing that uncertainty through additional data collection 
and evaluation of outcomes is thus essential for effective management [@walters1978].  
...
<!--Gonna be two examples.  Shorten me? -->

...
Yet recent mathematical breakthroughs such as @Holden2015, @Hennig2019, and @Henning2021 have finally been able to extend Reed's theorem to such cases more generally. 
...
That simple intuition does not extend to the more complex cases examined by @Hening2021 and certainly in even richer models where the optimal policy is not tractable.
Consequently, these results do not provide a short-cut for finding an optimal policy without knowing the true model: we cannot merely look for the harvest that generates the peak growth rate (even in Reed's case that would not be optimal due to the role of discounting).
All the same, they are a reminder of why simple, processed based models can lead us to the optimal policy without providing accurate forecasts.
Meanwhile, filtering out or weighing models based on forecast skill _alone_ can precipitate the forecast trap by decreasing any prior belief in such simple but effective models in favor of those with greater forecast skill.
-->
...
Buoyed by the rapid expansion of available data and computational power, complex and increasingly opaque models are becoming more common [].
...
I hope this simple and intuitive logic of Figure 4 will provide a ready reminder as to why the model that produces the best forecast will not always produce the best decision.

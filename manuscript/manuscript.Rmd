...
That solution was first provided by @Reed1979, who demonstrated the optimal policy would be a so-called "bang-bang" strategy, in which the manager seeks to bring the fish stock to some target biomass as quickly as possible whenever it deviates from that size. 
The precise value of that target biomass depends of course on the details of the model -- but importantly not all the details.
...
While the SDP is able to discover this same policy numerically by examining the forecast probabilities, Reed's theorem makes it intuitive why that policy is shared between models 1 and 3.

However, Reed's proof considered only one-dimensional growth models $f(x)$, and so tells us nothing about models with stage structure or interacting species.
It is easy to believe that the simple and thus non-unique optimal policy studied by @Reed1979 is an artifact of the simple growth model: surely the intuition provided will not extend to two or more species? 
Fortunately, recent work in the mathematics literature has finally been able to extend Reed's results.
@Hening2021 proves under comparable assumptions that the optimal strategy

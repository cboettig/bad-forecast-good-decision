...
I have used the model parameters from @Brias2021, which were selected to ensure co-existence of the five species as outlined in their paper.
Following @Brias2021, we assume three species of herring, $X_1$, $X_2$, $X_3$ are preyed upon by 

\begin{align}
x_{1,t+1} &= s_{1,t} \exp{(1.0213 -        s_{1,t} - 0.0861 s_{2,t} - 0.3141 s_{3,t} - 0.7252 s_{4,t} -0.2445 x_{5,t} +\epsilon_{1,t} )} \\
x_{2,t+1} &= s_{2,t} \exp{(1.0289 - 0.4765 s_{1,t} -        s_{2,t} - 0.1370 s_{3,t} - 0.9811 s_{4,t} -0.0915 x_{5,t} +\epsilon_{2,t} )} \\
x_{3,t+1} &= s_{3,t} \exp{(1.0207 - 0.3193 s_{1,t} - 0.3461 s_{2,t} -        s_{3,t} - 0.6367 s_{4,t} -0.8716 x_{5,t} +\epsilon_{3,t} )} \\
x_{4,t+1} &= s_{4,t} \exp{(         0.7252 s_{1,t} + 0.9811 s_{2,t} + 0.6367 s_{3,t} -        s_{4,t}                 +\epsilon_{4,t})} \\
x_{5,t+1} &= x_{5,t} \exp{(         0.2445 s_{1,t} + 0.0915 s {2,t} + 0.8716 s {3,t} -                        x_{5,t} +\epsilon_{5,t} )}
\end{align}
...
$$
\epsilon_{i,t} \stackrel{iid}{\sim} \mathcal{N}(0, \sigma_i)
$$
...
As with the original @Brias2021 model, parameters are not based directly on historical fisheries data, but have been selected to be consistent with simulations of the observed variables from the "true" model of @Brias2021, bass biomass $x_4$ and herring biomass $x_5$.


\begin{align}
x_{1,t+1} &= s_{1,t} \exp(3 - 8 s_{1,t} - 0.9 s_4 - 0.9 * x_{5,t} + \epsilon_{1,t} ) \\
x_{4,t+1} &= s_{4,t} \exp(0.1 s_{1,t} - s_{4,t} + \epsilon_{4,t}  ) \\
x_{5,t+1} &= x_{5,t} \exp(0.3 s_{1,t} - x_5  + \epsilon_{5,t}  )
\end{align}
...
\begin{align}
x_{1,t+1} &= s_{1,t} \exp(0.1 - 2.00 s_{1,t}  + \epsilon_{1,t} ) \\
x_{4,t+1} &= s_{4,t} \exp(1.45 - 2.75 s_{1,t}  + \epsilon_{4,t}  ) \\
...
knitr::opts_chunk$set(cache=TRUE, tidy = "styler", message = FALSE, warning = FALSE, echo = TRUE)
...
model_5sp <- function(u_prey = 0.5,
    ...
    for(r in 1:reps){
      ...
      for (t in 1:(Tmax-1)) {
          ...
          x1[t+1] = s1[t]*exp(1.0213 - s1[t] - 0.0861*s2[t] -
                                0.3141*s3[t] - 0.7252*s4[t] -
                                0.2445*x5[t] + xi1[t])
          x2[t+1] = s2[t]*exp(1.0289 - 0.4765*s1[t] - s2[t] -
                                0.1370*s3[t] - 0.9811*s4[t] -
                                0.0915*x5[t] + xi2[t])
          x3[t+1] = s3[t]*exp(1.0207 - 0.3193*s1[t] - 
                                0.3461*s2[t] - s3[t] -
                                0.6367*s4[t] - 0.8716*x5[t] + xi3[t])
          x4[t+1] = s4[t]*exp(0.7252*s1[t] + 0.9811*s2[t] +
                                0.6367*s3[t] - s4[t] + xi4[t])
          x5[t+1] = x5[t]*exp(0.2445*s1[t] + 0.0915*s2[t] + 
                                0.8716*s3[t] - x5[t] +  xi5[t])
...
```{r}
ratio <- mean_utility_A / mean_utility_B
```

The ratio of mean utility under model A to mean utility under the nearly-optimal model B is `r round(ratio *100)`%.

...
n_models <- 30
rs <- seq(0.05, 2, length.out = n_models)
Ks <- seq(0.1, 10*8/5, length.out = n_models)
...
d  %>%
  pivot_longer(names(model_set[-1]), "model") %>%
  mutate(model = as.numeric(model)) %>%
  ggplot(aes(state, value, col = model, group=model)) +
...
```{r}
best_model <- which.max(am_many$posterior[20,])
```

```{r, fig.cap = ""}
am_many$df %>% 
  mutate(belief = am_many$posterior[,best_model]) %>%
...
```{r}
am_npv <- sum(am_many$value * discount ^ am$time)
am_npv / npv[[1,"npv"]]
```

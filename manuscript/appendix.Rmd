...
```{r plot_many_models, fig.width=6, fig.height=4}
final_belief <- t(am_many$posterior)
final_belief <- final_belief[,dim(final_belief)[[2]]]
posteriors <- data.frame(model = as.character(1:length(many_models)), belief = final_belief)
...
d <- 
  ...
  mutate(state = states) %>% pivot_longer(names(model_set[-1]), "model") %>%
  left_join(posteriors)

d %>% 
  ggplot(aes(state, value, group=model, alpha = belief)) +
  #geom_hline(aes(yintercept = 0), lwd=1, lty=2) + 
  geom_line(lwd=1.5) + 
  geom_line(aes(state, true, alpha=1), col = "red", show.legend = FALSE) +

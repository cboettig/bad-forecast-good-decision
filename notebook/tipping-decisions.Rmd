...
actions <- seq(0,1, length.out = 100)
observations <- states
sigma_g <- 0.05
sigma_m <- 0.0
...
may <- 
  function(y, h=0){ # May
    ...
    pmax(
      ...
      y + y * (p$r+h) * (1 - y / p$K)  - (p$a + h) * y ^ p$q / (y ^ p$q + p$b ^ p$q),
      0)
  }
...
may_2 <- function(x, h = .5) may(x,h)
may_10 <- function(x, h = 1) may(x,h)
```
...
landscape <- map_dfr(list(logistic_1, may, may_2, may_10), 
                     function(f) tibble(state = states, f = f(states) -states),
                     .id = "model")

landscape %>% ggplot(aes(state, f, col=model)) + geom_line()
```
...
```{r}
matrices <- lapply(list(logistic_1, may), 
...
```{r, results="hide"}
#solns <- lapply(matrices, function(m) mdp_value_iteration(m$transition, m$reward, discount, epsilon = 0.01, max_iter = 5000))
...
```{r}
df %>% ggplot(aes(state, action, col=model)) + geom_line(alpha=0.5) 
...
sim %>% mutate(state = states[state], action = 20*actions[action]) %>% 

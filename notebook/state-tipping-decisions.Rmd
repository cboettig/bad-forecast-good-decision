...
landscape <- map_dfr(list(logistic_1, alt, may), 
                     function(f) tibble(state = states, f = f(states) -states),
                     .id = "model")

landscape %>% ggplot(aes(state, f, col=model)) + geom_point() + ylim(-5,10) + xlim(0,5)
```
...
```{r}
matrices <- lapply(list(logistic_1, alt, may), 
...
```{r}
nothing <- rep(1, length(states))
m <- matrices[[3]]
x0 = which.min(abs(states - 17))
sim <- mdp_planning(m$transition, m$reward, discount, model_prior = c(1), 
                   policy = nothing, x0 = x0, Tmax = 1000)
sim %>% mutate(state = states[state], action = actions[action]) %>% 
  ggplot(aes(time, state)) + geom_point()+geom_path() + 
  geom_point(aes(time, action), col="blue")

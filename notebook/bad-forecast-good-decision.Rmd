...
```{r}
states <- seq(0,25, length.out = 200)
actions <- states
obs <- states
sigma_g <- 0.05
...
transitions <- lapply(seq_along(models), function(i) transition_matrices(models[[i]], states, actions, model_sigmas[[i]]))
...
compare %>% filter(model < 3) %>% 
  select(time, model, true, expected, low, high) %>% 
  ggplot(aes(time, col = model, fill = model)) + 
  geom_point(aes(y = true), pch = "*", size = 10) +

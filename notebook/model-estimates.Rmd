...
may  <- nimble::nimbleCode({
  ...
  q <- 3
  s <- 2
  r ~ dlnorm(log(.7) - s^2/2, s)
  K ~ dlnorm(log(1.2)- s^2/2, s)
  a ~ dlnorm(log(0.2) - s^2/2,  s)
  b ~ dlnorm(log(0.15) - s^2/2, s)
...
l_draws %>% summarise(across(everything(), list(mean = mean, sd = sd)))
...
m_draws %>% summarise(across(everything(), list(mean = mean, sd = sd)))
